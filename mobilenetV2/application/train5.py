...
# 用于训练修改网络结构后的mobilenetv2_4，区别就在于恢复网络权重时需要重新建立命名与数据的关联，见218行
# 相比train.py新增了cosLR, 预热，L2正则化; 相比train5.py修改了L2正则化的实现方式，修改为自定义的实现方式
...
class Regularization(torch.nn.Module):
    ...
    # 指定运行模式，经过测试发现，其是这个函数用不到，张量乘以标量，以及张量乘以单元素张量不需要考虑同设备问题，只有多维张量乘以多维张量才需要考虑设备问题
    def to(self, device):
        self.device = device
        super().to(device)
        return self
    ...
    def get_weight(self, model):
        ...
        for name, param in model.named_parameters():
            if 'weight' in name:                     # 仅仅包含卷积和全链接的w，而不包含bias，也不包含BN层
                weight_list.append((name, param))
        return weight_list
    # 计算张量范数
    ...
    def forward(self, model):
        ...
        return reg_loss
cost_reg = Regularization(model, weight_decay, p=2)
...
def train(model, train_loader, optimizer, epoch):
    sum_loss, sum_loss_reg = 0.0, 0.0  # 统计每个epoch的总loss和总正则化loss
    ...
    for steps, (inputs, labels) in enumerate(train_loader):
        ...
        loss, loss_reg = cost(outputs, labels), cost_reg(model)   # 计算损失
        if weight_decay == 0:
            loss.backward()            # 损失梯度反向传输，损失梯度结合缓存的中间层输出计算出所有权重参数的梯度
        else:
            (loss + loss_reg).backward()                          # 加入正则化损失
    ...
    return sum_loss/len(train_loader), sum_loss_reg/len(train_loader), train_correct/sum_image  # 返回值都是tensor类型
...
def test(model, test_loader):
    ...
    with torch.no_grad():  # 放弃梯度记录节省内存，等效于设置requires_grad=False，正向传播时不记录中间层结果，无法进行梯度计算和更新参数
        ...
        for inputs, labels in test_loader:
            ...
            loss_reg = cost_reg(model)     # 计算正则化损失
            _, id = torch.max(outputs.data, 1)
            sum_loss += loss.data
            sum_loss_reg += loss_reg.data
            test_correct += torch.sum(id == labels.data)
            sum_image += id.size()[0]
    return sum_loss/len(test_loader), sum_loss_reg/len(test_loader), test_correct/sum_image
...
# 预先计算loss和acc
loss0, loss0_reg, acc0= test(model, test_loader)
print("initial loss: {:.4f} | {:.4f}".format(loss0, loss0_reg))
...
for epoch in range(initial_epochs) :
    ...
    loss, loss_reg, acc = train(model, train_loader, optimizer, epoch)
    val_loss, val_loss_reg, val_acc = test(model, test_loader)
    model_name = save_model(epoch+1, acc, val_acc, "./tmp/", dataset_name, model, alpha)
    val_acc_list.append(val_acc)
    val_acc_dict[epoch] = model_name
    print("epoch: %d, loss: %0.5f, loss_reg: %0.5f, acc: %0.4f, val_loss: %0.5f, val_loss_reg: %0.5f, val_acc: %0.4f" % (epoch+1, loss, loss_reg, acc, val_loss, val_loss_reg, val_acc))
    print("")

# 计算loss和acc
loss1, loss1_reg, acc1 = test(model, test_loader)
print("middle loss: {:.4f} | {:.4f}".format(loss1, loss1_reg))
print("middle accuracy: {:.4f}".format(acc1))
print("")

# 解冻所有层
for name, param in model.named_parameters(): param.requires_grad = True
optimizer = torch.optim.Adam(model.parameters(), lr=second_learning_rate)  # 优化器和损失函数不需要tocuda()
...
# 计算loss和acc
loss2, loss2_reg, acc2 = test(model, test_loader)
print("last loss: {:.4f} | {:.4f}".format(loss2, loss2_reg))
