...
modeltype = "50"
dataset_name = "ShuZiComp"
train_dir = "./dataset/ShuZiComp/train.txt"
test_dir = "./dataset/ShuZiComp/test.txt"
output_path = "./checkpoint/pth3/"
...
model.fc = nn.Linear(num_ftrs, CLASS_NUM, bias=True)
model.to(device)
#print(model)
...
# 保存模型参数和结构
def save_model(epoch, acc1, acc2, path, dataset_name, model, alpha, modeltype):
    localtime = time.strftime("%Y%m%d-%H%M", time.localtime())
    output_model = path + "{}_resnet{}-v0.1_{}_{}_{}_{}_{:.4f}_{:.4f}.pth".format(dataset_name, modeltype, localtime, IMG_SIZE, alpha, epoch, acc1, acc2)
...
for epoch in range(initial_epochs) :
    ...
    save_model(epoch+1, acc, val_acc, "./tmp/", dataset_name, model, alpha, modeltype)
...
# 保存模型参数和结构
save_model(initial_epochs+second_epochs, acc, acc2, output_path, dataset_name, model, alpha, modeltype)
