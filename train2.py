# 用于训练alpha不等于1的模型，通过修改冻结的层数实现，本代码创建的原因是，经网络下载的对应0.75的模型参数文件与自定义的模型，在最后一个卷积层的通道数上不一致，所以选择不恢复这个卷积层的权重
import torch
...
train_dir = "./dataset/8cls_13k_2k_80k_9k/train.txt"
test_dir = "./dataset/8cls_13k_2k_80k_9k/test.txt"
output_path = "./checkpoint/pth2/"
weight_path = "./checkpoint/mobilenetv2_0.75-dace9791.pth" # 非官方权重，官方没有0.75的权重，所以命名方式不一样，需要调整命名，甚至修改模型结构
